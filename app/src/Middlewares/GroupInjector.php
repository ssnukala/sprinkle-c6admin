...
/**
 * Group Injector Middleware.
 * 
 * Extends CRUD6Injector to work with sprinkle-admin style routes.
 * Injects the 'groups' model name into the route before passing to CRUD6Injector.
 */
class GroupInjector extends CRUD6Injector
{
    /**
     * @var string Placeholder name for group slug in route
     */
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if ($route) {
            $route->setArgument('model', 'groups');
        }
        
        // Set the model name for getInstance method
        $this->currentModelName = 'groups';
        ...
        // Get schema
        $schema = $this->schemaService->getSchema('groups', null);
