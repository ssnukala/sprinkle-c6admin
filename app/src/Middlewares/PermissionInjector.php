...
/**
 * Permission Injector Middleware.
 * 
 * Extends CRUD6Injector to work with sprinkle-admin style routes.
 * Injects the 'permissions' model name into the route before passing to CRUD6Injector.
 */
class PermissionInjector extends CRUD6Injector
{
    /**
     * @var string Placeholder name for permission slug in route
     */
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if ($route) {
            $route->setArgument('model', 'permissions');
        }
        
        // Set the model name for getInstance method
        $this->currentModelName = 'permissions';
        ...
        // Get schema
        $schema = $this->schemaService->getSchema('permissions', null);
